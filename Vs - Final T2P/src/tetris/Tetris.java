...
public class Tetris {
	 ...
	 * Magenta = T
	 */
	private static final Color[] c = { Color.GRAY, Color.YELLOW, Color.CYAN, Color.BLUE, Color.ORANGE,
	...
	public void displayUI(Graphics gi) {
		gi.setColor(UIColor);
		gi.drawString("LINES CLEARED: " + linesCleared, panelC + 10, panelR + 15);
		gi.drawString("CURRENT LEVEL: " + level, panelC + 130, panelR + 15);
		if (isPaused)
			gi.drawString("PAUSED", panelC + 10, 30);
		if (isGameOver)
			gi.drawString("GAMEOVER -- Q FOR QUIT; R FOR RESTART", panelC + 10, panelR + 40);
		gi.drawString("HOLD", panelC + 300, panelR + 290);
		gi.drawString("NEXT", panelC + 300, panelR + 40);
		for (int k = 0; k < 5; k++) {
			for (int i = 0; i < 2; i++) {
				for (int j = 0; j < 4; j++) {
