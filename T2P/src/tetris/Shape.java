...
public class Shape {
        ...
        if (collision && timePassedFromCollision > 500) {
            for (int row = 0; row < coords.length; row++) {
                for (int col = 0; col < coords[0].length; col++) {
                    if (coords[row][col] != 0) {
                        board.getBoard(player)[y + row][x + col] = color;
                    }
                }
            }
            checkLine();
            board.addScore(player);
            board.setCurrentShape(player);
        ...
        if (!(x + deltaX + coords[0].length > 10) && !(x + deltaX < 0)) {
            ...
            for (int row = 0; row < coords.length; row++) {
                for (int col = 0; col < coords[row].length; col++) {
                    if (coords[row][col] != 0) {
                        if (board.getBoard(player)[y + row][x + deltaX + col] != null) {
                            moveX = false;
                        }
                ...
                for (int row = 0; row < coords.length; row++) {
                    for (int col = 0; col < coords[row].length; col++) {
                        if (coords[row][col] != 0) {
                            ...
                            if (board.getBoard(player)[y + 1 + row][x + col] != null) {
                                collision();
                            }
                        }
                    }
    ...
    public void render(Graphics g, int offsetX) {
        ...
        for (int row = 0; row < coords.length; row++) {
            for (int col = 0; col < coords[0].length; col++) {
                if (coords[row][col] != 0) {
                    g.fillRect(offsetX + col * 30 + x * 30, row * 30 + y * 30, Board.blockSize, Board.blockSize);
                }
            }
        }
    }
    ...
    private void checkLine() {
        int size = board.getBoard(player).length - 1;

        for (int i = board.getBoard(player).length - 1; i > 0; i--) {
            int count = 0;
            for (int j = 0; j < board.getBoard(player)[0].length; j++) {
                if (board.getBoard(player)[i][j] != null) {
                    count++;
                }

                board.getBoard(player)[size][j] = board.getBoard(player)[i][j];
            }
            if (count < board.getBoard(player)[0].length) {
                size--;
            }
        }
    }
        ...
        for (int row = 0; row < rotatedShape.length; row++) {
            for (int col = 0; col < rotatedShape[row].length; col++) {
                if (rotatedShape[row][col] != 0) {
                    if (board.getBoard(player)[y + row][x + col] != null) {
                        return;
                    }
                }
            }
